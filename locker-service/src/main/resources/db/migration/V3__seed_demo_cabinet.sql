...
INSERT INTO locker_schema.lockers (store_id, code, name, status, address, latitude, longitude, description, landing_pad, landing_marker_id)
SELECT 1, 'CAB-DEMO-01', 'Tu demo capstone 3x3 + vali', 'ACTIVE',
       'FPT University HCMC', 10.8411, 106.8097,
       'Tu demo theo ban ve: 1.5x1.2x0.5m, bai dap drone tren noc, hang 1 danh cho drone',
       TRUE, 'ARUCO-23'
WHERE NOT EXISTS (SELECT 1 FROM locker_schema.lockers WHERE code = 'CAB-DEMO-01');

WITH demo AS (SELECT id FROM locker_schema.lockers WHERE code = 'CAB-DEMO-01')
INSERT INTO locker_schema.locker_boxes (locker_id, box_number, size, status, cell_type, row_index, col_index, description)
SELECT demo.id, v.box_number, v.size, 'AVAILABLE', v.cell_type, v.row_index, v.col_index, v.description
FROM demo,
     (VALUES
        (1, 'MEDIUM', 'DRONE',    1, 1, 'O nhan hang drone - hang 1 cot 1'),
        ...
        (10, 'XL',    'XL',       2, 0, 'O vali 0.30x0.80x0.40 - khu ben trai')
     ) AS v(box_number, size, cell_type, row_index, col_index, description)
WHERE NOT EXISTS (
    SELECT 1 FROM locker_schema.locker_boxes b WHERE b.locker_id = demo.id
);
