...
| Flutter stores (customer) | DONE | Feature `lib/features/stores/**`: list (search + nearby), detail (info + ratings + directions), entry "Khám phá cửa hàng" từ home; gọi `/api/stores`, `/api/stores/{id}`, `/api/stores/{id}/ratings`. Reskin 2026-06-14 theo brand RN: list card có ảnh/badge/tim, detail hero ảnh + back/tim, thêm màn **Yêu thích** (`favorites_page`, route `/stores/favorites`, local `shared_preferences`). `flutter analyze` 0 error. | Manual smoke trên emulator với deploy có store data. |
...
- Port UI role **user** từ app React Native cũ sang Flutter (2026-06-14, branch `feat/mobile-user-ui-revamp`): thêm UI kit thương hiệu dùng chung `lib/shared/widgets/user_ui_kit.dart` (palette navy gradient `#003D5B→#0077B6→#00B4D8`, `BrandHeroHeader`, `BrandAvatar`, `BrandFilterChip`, `BrandSectionHeader`, `BrandStatusBadge`, `BrandCircleIconButton`); reskin Home (gradient header + avatar/bell, filter chips, welcome card, lưới dịch vụ, card cửa hàng có ảnh/badge/tim), thanh điều hướng dưới dạng pill navy nổi (giữ nút QR giữa), Cửa hàng + chi tiết cửa hàng (hero ảnh + back/tim) + danh sách Tủ, Hồ sơ (header hero gradient), Ưu đãi, Đơn tủ, Thông báo về cùng design system. Thêm màn **Yêu thích cửa hàng** (`favorites_page` + route `/stores/favorites`, lưu local bằng `shared_preferences`). Chỉ reskin UI + dùng lại service/route sẵn có (không đổi nghiệp vụ/endpoint). `flutter analyze` 0 error.
...
|---|---|---|---|---|
| 2026-06-14 | Flutter (UI user revamp) | `flutter analyze --no-pub` cho từng nhóm màn (home/bottom-nav/kit; stores/detail/favorites/locker/router; profile_header/vouchers; orders/notifications) rồi `flutter analyze --no-pub` toàn project. | PASS | Từng nhóm "No issues found". Full project: 407 issue (tất cả info/warning debt cũ; **0 error** — lọc `error -`/`error •` = 0 dòng), giảm từ ~412 nhờ dọn `withOpacity` ở file đã chạm. Branch `feat/mobile-user-ui-revamp`. Chưa chạy emulator smoke/`build apk` phiên này. |
...
|---|---|---|---|---|
| 2026-06-14 | Claude | Port giao diện role **user** từ app React Native cũ (`laundry-locker-frontend/mobile`) sang app Flutter chính bằng cách **dựng lại** thiết kế (khác framework nên không copy code). Thêm UI kit thương hiệu (`user_ui_kit.dart`: palette navy `#003D5B→#0077B6→#00B4D8`, `BrandHeroHeader`/`BrandAvatar`/`BrandFilterChip`/`BrandSectionHeader`/`BrandStatusBadge`/`BrandCircleIconButton`). Reskin: Home (gradient header + avatar/bell, chips, welcome card, dịch vụ, card cửa hàng), bottom nav navy nổi (giữ nút QR giữa), Cửa hàng + chi tiết (hero ảnh + back/tim) + Tủ, Hồ sơ (hero gradient), Ưu đãi, Đơn tủ, Thông báo. Thêm màn **Yêu thích cửa hàng** (`favorites_page`, lưu local qua `shared_preferences`) + route `/stores/favorites`. Giữ nguyên data/service/route nghiệp vụ. | branch `feat/mobile-user-ui-revamp`: `lib/shared/widgets/{user_ui_kit.dart(mới),custom_bottom_navigation_bar.dart}`, `lib/features/home/.../home_page.dart`, `lib/features/stores/**` (stores/store_detail/favorites(mới)/store_card/favorite_stores_service(mới)), `lib/features/locker/.../locker_page.dart`, `lib/features/profile/.../profile_header.dart`, `lib/features/vouchers/.../my_vouchers_page.dart`, `lib/features/locker_ops/.../my_locker_orders_page.dart`, `lib/features/notifications/.../notification_list_page.dart`, `lib/core/routing/app_router.dart`; living docs + mirror. KHÔNG commit `.env`/`env_config.g.dart`. | `flutter analyze` targeted từng nhóm = **No issues found**; full project = **0 error** (407 info/warning debt cũ, giảm so với ~412). Chưa smoke emulator phiên này. |
