...
|---|---|---|---|---|---|
| 2026-06-16 | `chore/real-data-seed` | Tạo seed dữ liệu thật (Tiếng Việt có dấu) trên server production. Đổi tên/email/địa chỉ/mô tả thay vì "demo". | Thêm `scripts/seed-real-data-ms.sql` (sinh từ script cũ) và chạy thẳng trên droplet `146.190.84.136`. | Không sửa migration/schema, không sửa code. | Thay đổi data trên production server thành dữ liệu trông như thật. Đã chạy PASS trực tiếp trên server qua SSH `docker exec ll-ms-postgres`. |
...
| Flutter customer locker ops | DONE | Login/home quick actions/rental/send/my-orders smoke pass. UI 3 màn revamp về design system shadcn navy + action gate theo state machine; chi tiết đơn tủ có nút chỉ đường tới locker qua Google Maps/address fallback. **2026-06-15**: Lưới ô tủ 2D (`StoreLockerGridPage`) — lazy-load layout theo store, DRONE cell icon + màu indigo + non-tappable; booking bỏ picker khi đến từ lưới ô (skip API load, hiện card read-only). `flutter analyze` 0 error. | Smoke trên emulator với deploy cho UI mới (create/confirm/complete/extend/delegate/report/directions) + lưới ô tủ. |
...
|---|---|---|---|---|
| 2026-06-16 | DB / Prod data seed | Chạy `seed-real-data-ms.sql` trên Postgres Droplet `146.190.84.136` | PASS | SSH thành công, đã copy script tự động chuyển đổi chuỗi "demo" thành tên/địa chỉ/mô tả Tiếng Việt thật và chạy qua `docker exec ll-ms-postgres`. Dữ liệu 100 bản ghi đã được cập nhật. |
