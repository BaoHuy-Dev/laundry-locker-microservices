...
- `PARTIAL_EXTERNAL`: compileable implementation exists, but provider/device credentials or external protocols are
  environment dependent.
- `PARTIAL_SOURCE_TODO`: the monolith itself contains TODO/mock/baseline behavior, and the microservice keeps that same
  maturity level.
