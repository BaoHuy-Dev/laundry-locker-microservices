...
- `docker-compose.yml` map gateway `"${API_GATEWAY_PORT:-18080}:8080"` → mặc định host **18080**. Để client ngoài vào được `:8080` (đúng port firewall mở), droplet **phải** đặt `API_GATEWAY_PORT=8080` trong env/`.env` của compose. Health-check trong `deploy-from-artifact.sh` dò `http://127.0.0.1:8080/actuator/health` nên droplet vốn được kỳ vọng chạy gateway ở host 8080.
- **Tình trạng khi kiểm tra 2026-06-16**: SSH `:22` trả banner OK, nhưng `:8080` trả "connection refused" (host sống, không có listener) ⇒ stack backend đang **không chạy** trên droplet. CPU droplet quan sát 99.8% (vừa resize/restart). Cần SSH vào droplet và `docker compose up -d` (hoặc chạy lại deploy workflow) để bật lại gateway thì FE/mobile mới kết nối được. Việc bật server nằm ngoài quyền của agent (harness chặn ghi prod).
