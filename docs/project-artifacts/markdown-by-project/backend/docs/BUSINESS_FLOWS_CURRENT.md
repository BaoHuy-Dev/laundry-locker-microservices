...
**Phase 1 — Maintenance queue + mission foundation (2026-07-14):** queue điều phối mới dùng **`orderId`** thay vì `DroneDeliveryRequest.id`: `GET /api/maintenance/drone-orders`, `POST /api/maintenance/drone-orders/{orderId}/accept`, `POST /api/maintenance/drone-orders/{orderId}/launch`. `accept` không yêu cầu thanh toán trước; drone vẫn phải active, `IDLE`, pin `>20%`. Với `fulfillmentMode=STANDARD`, locker đích phải `ACTIVE`, có landing pad và trạng thái pad `OK`; với `DEMO`, backend dùng source locker cấu hình và chỉ yêu cầu locker đích `ACTIVE`, cho phép adapter mô phỏng thay hardware pad. `accept` tạo/ghi `drone_missions` (migration **`V8__drone_mission_foundation.sql`**), còn `launch` đổi mission/order sang `LAUNCHING` và yêu cầu locker-service đổi drone sang `IN_FLIGHT`. Gateway route `/api/maintenance/drone-orders/**` tới order-service. Mobile MAINTENANCE dùng 3 nhóm `AWAITING_DISPATCH` / `ACCEPTED` / `LAUNCHING` với action `Tiếp nhận` / `Phóng`. Legacy `/api/maintenance/drone-deliveries` chỉ còn cho dữ liệu/flow cũ.
