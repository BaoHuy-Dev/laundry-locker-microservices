...
**Phase 0 — Order foundation (2026-07-14):** customer booking drone không còn đi qua flow local/mock `DRN-*`. `order-service` đã có `POST /api/orders/drone-deliveries` (header `Idempotency-Key`) để tạo **đơn thật** `type=DRONE_DELIVERY`, reserve ô `DRONE` qua `channel=DRONE`, trả về **một `orderId` thật** và `reservedBoxId`. Đọc chi tiết tối thiểu bằng `GET /api/orders/{orderId}/drone-delivery`; mobile booking sheet chờ response backend rồi mới báo thành công, không còn tự mở `DroneTrackingPage` giả. Hủy trước điều phối vẫn dùng `PUT /api/orders/{orderId}/cancel`. Migration order-service cho phase này là **`V7__drone_delivery_order_foundation.sql`**. Legacy `POST /api/drone-deliveries` còn tồn tại tạm để tương thích/manual test cũ, nhưng không còn là flow chính của mobile.

**Phase 1 — Maintenance queue + mission foundation (2026-07-14):** sau khi payment event hoàn tất cho order `DRONE_DELIVERY`, `order-service` chuyển order sang `status=AWAITING_DISPATCH`, `deliveryStage=AWAITING_DISPATCH`. Queue điều phối mới dùng **`orderId`** thay vì `DroneDeliveryRequest.id`: `GET /api/maintenance/drone-orders`, `POST /api/maintenance/drone-orders/{orderId}/accept`, `POST /api/maintenance/drone-orders/{orderId}/launch`. `accept` tạo/ghi `drone_missions` (migration **`V8__drone_mission_foundation.sql`**) và chạy preflight skeleton qua internal calls sang `locker-service`: drone phải `active=true`, `status=IDLE`, pin `>20%`; locker đích phải `ACTIVE`, có `landingPad=true`, `landingPadStatus=OK`. `launch` hiện mới là skeleton: đổi mission sang `LAUNCHING`, order sang `deliveryStage=LAUNCHING`, và yêu cầu locker-service đổi drone sang `IN_FLIGHT` qua `/internal/drones/{id}/status`. Gateway thêm route riêng `/api/maintenance/drone-orders/** -> order-service` trước route maintenance chung. Mobile MAINTENANCE đã được cắt sang queue mới với 3 nhóm `AWAITING_DISPATCH` / `ACCEPTED` / `LAUNCHING` và action `Tiếp nhận` / `Phóng`. Legacy `/api/maintenance/drone-deliveries` vẫn còn tạm cho dữ liệu/flow cũ nhưng không còn là queue chính cho maintenance Phase 2.

**Phase 1 — Push notification + timeline (2026-07-03):** backend đẩy FCM data payload `{ type, title, content, orderId, deliveryId, status, eta }` với `type` ∈ `drone_dispatched|drone_approaching|drone_arrived|drone_delivered|drone_delayed|drone_failed` (`status` cùng bộ tên). Mobile tap noti → trang timeline `DroneDeliveryTrackingPage(orderId)` (route `/drone-delivery`). Tái dùng channel FCM `aisl_high_importance_channel`. Endpoint fetch trạng thái `GET /api/orders/{orderId}/drone-delivery` **đã có ở backend từ 2026-07-14**, nhưng hiện mới trả read model foundation (`orderId`, `reservedBoxId`, `deliveryStage=PENDING_PAYMENT`, ...), chưa phải timeline đầy đủ 6 mốc nên mobile timeline feature vẫn giữ cờ `droneDeliveryEnabled=false` + mock cho đến phase tracking.
