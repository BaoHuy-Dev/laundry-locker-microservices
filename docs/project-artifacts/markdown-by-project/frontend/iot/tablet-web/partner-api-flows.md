...
> ⚠️ **Lưu ý:** Sau khi đăng ký, Partner ở trạng thái `PENDING`. Admin phải duyệt (`APPROVED`) thì Partner mới có thể sử
> dụng các API quản lý.
...
> 🔑 **Quan trọng:** `accessCode` = "STAFF-8A3F2B" chính là mã OTP mà Staff sẽ dùng để mở tủ locker. Partner gửi mã này
> cho Staff (qua app, SMS, hoặc chat).
...
- **Mô tả:** Partner đánh dấu đơn đã hoàn thành giặt → Hệ thống tự động tạo **mã Access Code (OTP)** cho Staff mở tủ trả
  đồ.
- **Role:** `PARTNER`
