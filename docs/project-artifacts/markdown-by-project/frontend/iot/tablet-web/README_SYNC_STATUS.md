...
> **Cập nhật:** 2026-02-23  
> **Mục đích:** Tài liệu tổng hợp cho team Frontend & Backend biết trạng thái đồng bộ API, những gì đang hoạt động, và
> những gì cần sửa/thêm.
...
- **Vấn đề:** Backend dùng `box.getLocker().getCode()` (= `LOC-01-001`) làm deviceId, ESP trước đây dùng
  `ESP8266_LOCKER_01`
...
- **Đề xuất Backend:** Bỏ `@NotNull` trên `boxId` trong `VerifyPinRequest.java` và `UnlockBoxRequest.java`, để backend
  tự tìm box qua PIN code (giảm 1 API call)
...
- **Ghi chú MQTT:** Backend gửi `{"box_id": 3, "action": "OPEN"}` — ESP chỉ xử lý nếu `box_id == BOX_ID`, nên sẽ bỏ qua
  box khác
