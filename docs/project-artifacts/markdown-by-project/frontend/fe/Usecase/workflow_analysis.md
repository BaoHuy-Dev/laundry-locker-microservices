...
> [!IMPORTANT]
> Staff **không có tài khoản** trong hệ thống. Họ tương tác với tủ đồ thông qua **mã truy cập (AccessCode)** do Partner
> tạo ra. Đây là thiết kế cốt lõi để nhân viên thực địa không cần đăng nhập.
...
**NotificationType:** `ORDER_STATUS` · `PICKUP_REMINDER` · `AUTO_CANCEL` · `PAYMENT` · `SYSTEM` · `PROMOTION` ·
`LOYALTY_REWARD`
...
**Mô tả chung:** Khách hàng chọn dịch vụ (gửi đồ hoặc giặt), tạo đơn hàng, nhận mã PIN, đến tủ mở bằng PIN, bỏ đồ vào và
xác nhận.
...
`PricingType`, `ServiceCategory`

---

#### Role: USER

**Context:** Khách mở app, chọn locker, chọn dịch vụ, tạo đơn hàng. Hệ thống cấp mã PIN. Khách đến tủ, nhập PIN trên
tablet, mở tủ, bỏ đồ vào, xác nhận.
...
> [!NOTE]
> Nếu khách **không xác nhận** trong 30
> phút, [OrderSchedulerService](file:///d:/BigProject/laundry-locker-backend/laundry-locker-backend/src/main/java/com/huynqb/laundrylockerbackend/core/scheduler/OrderSchedulerService.java#25-278)
> tự động hủy:
...
**Mô tả chung:** Khi đơn ở trạng thái WAITING, Partner tạo mã AccessCode (COLLECT) cho Staff. Staff đến tủ, nhập mã trên
tablet, tủ mở ra, Staff lấy đồ mang về cửa hàng.
...
**Mô tả chung:** Sau khi thu gom, Staff mang đồ về cửa hàng. Partner cập nhật trạng thái → PROCESSING, cân/đếm đồ thực
tế, tính giá chính xác (với dịch vụ LAUNDRY).
...
**Mô tả chung:** Khi đồ giặt xong (READY), Staff dùng mã RETURN để mở tủ locker, bỏ đồ vào, hệ thống cập nhật trạng thái
và sinh mã PIN mới cho khách.
...
**Mô tả chung:** Khách nhận thông báo, đến tủ nhập PIN lấy đồ. Có 2 cách hoàn thành: (a) tự xác nhận qua app, (b) thanh
toán online xong tự hoàn thành.
...
> [!CAUTION]
> Nếu khách **lấy đồ trễ hơn 24 giờ** kể từ khi trả vào tủ (`pickupDeadline`), hệ thống tự động tính phí trễ hạn cộng
> vào đơn hàng.
...
**Enum sử dụng:** `PaymentStatus`,
`PaymentMethod`, [OrderStatus](file:///d:/BigProject/laundry-locker-backend/laundry-locker-backend/src/main/java/com/huynqb/laundrylockerbackend/module/order/service/OrderService.java#317-329)
...
> [!IMPORTANT]
> **MQTT là best-effort.** Nếu gửi lệnh MQTT thất bại, hệ thống KHÔNG rollback trạng thái đơn hàng. Lệnh mở tủ vẫn được
> coi là thành công ở tầng logic. ESP8266 có thể retry nhờ `automaticReconnect = true`.
