...
Hệ thống có 3 **scheduled task** chạy tự động để duy trì trạng thái nhất quán, cùng với các API cho Admin trigger thủ
công và Staff module cho nhân viên hệ thống.
...
> [!WARNING]
> **Master PIN** (`999999` mặc định) cho phép mở BẤT KỲ box nào. Config qua `app.staff.master-pin` trong
`application.properties`. Chỉ nên dùng cho nhân viên hệ thống đã xác thực.
