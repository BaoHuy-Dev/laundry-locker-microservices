...
| [02-backend-https-cors.md](02-backend-https-cors.md) | **Nginx + Let's Encrypt + ghim port gateway 18080 + CORS** (phần hay lỗi nhất) |
...
1. **Port gateway phải khớp Nginx.** Gateway publish `18080` (đã ghim trong `.env`),
   Nginx `proxy_pass …:18080`. Sai port → **502** (mà trình duyệt báo nhầm thành CORS).
