...
Contract review chung cho backend/mobile, payload mẫu, state machine, config và checklist test: [`DRONE_DELIVERY_DEMO_WORKFLOW.md`](DRONE_DELIVERY_DEMO_WORKFLOW.md).

**Phase 0 — Order foundation (2026-07-14):** customer booking drone không còn đi qua flow local/mock `DRN-*`. `order-service` đã có `POST /api/orders/drone-deliveries` (header `Idempotency-Key`) để tạo **đơn thật** `type=DRONE_DELIVERY`, reserve ô `DRONE` qua `channel=DRONE`, trả về **một `orderId` thật** và `reservedBoxId`. Từ slice `feat/drone-pay-before-pickup`, order drone mới **vào thẳng** `status=AWAITING_DISPATCH`, `deliveryStage=AWAITING_DISPATCH`, `paymentStatus=UNPAID` ngay sau khi tạo, để đội bay có thể tiếp nhận trước khi khách thanh toán. Đọc chi tiết tối thiểu bằng `GET /api/orders/{orderId}/drone-delivery`; mobile booking sheet chờ response backend rồi mới báo thành công, không còn tự mở `DroneTrackingPage` giả. Hủy trước điều phối vẫn dùng `PUT /api/orders/{orderId}/cancel`, nhưng chỉ còn hợp lệ khi `deliveryStage=AWAITING_DISPATCH` (chưa bị maintenance accept). Migration order-service cho phase này là **`V7__drone_delivery_order_foundation.sql`**. Legacy `POST /api/drone-deliveries` còn tồn tại tạm để tương thích/manual test cũ, nhưng không còn là flow chính của mobile.

**Phase 1 — Maintenance queue + mission foundation (2026-07-14):** queue điều phối mới dùng **`orderId`** thay vì `DroneDeliveryRequest.id`: `GET /api/maintenance/drone-orders`, `POST /api/maintenance/drone-orders/{orderId}/accept`, `POST /api/maintenance/drone-orders/{orderId}/launch`, và `POST /api/maintenance/drone-orders/{orderId}/cancel` (chỉ trước launch). `accept` không yêu cầu thanh toán trước; drone vẫn phải active, `IDLE`, pin `>20%`. Với `fulfillmentMode=STANDARD`, locker đích phải `ACTIVE`, có landing pad và trạng thái pad `OK`; với `DEMO`, backend dùng source locker cấu hình và chỉ yêu cầu locker đích `ACTIVE`, cho phép adapter mô phỏng thay hardware pad. `accept` tạo/ghi `drone_missions` (migration **`V8__drone_mission_foundation.sql`**), còn `launch` đổi mission/order sang `LAUNCHING` và yêu cầu locker-service đổi drone sang `IN_FLIGHT`. `cancel` giải phóng ô đã reserve, xóa mission và đưa order về `CANCELED`. Gateway route `/api/maintenance/drone-orders/**` tới order-service. Mobile MAINTENANCE dùng 3 nhóm `AWAITING_DISPATCH` / `ACCEPTED` / `LAUNCHING` với action `Tiếp nhận` / `Phóng`; accepted card có thêm hủy trước launch. Legacy `/api/maintenance/drone-deliveries` chỉ còn cho dữ liệu/flow cũ.

**Phase 1b — Drone fleet audit + pre-launch cancel reason (2026-07-16):** các action trong sheet drone maintenance trên mobile (`Nhận xử lý`, `Đổi trạng thái`, `Cập nhật pin %`, `Nhật ký bảo trì`) là endpoint thật dưới `/api/maintenance/drones/**`, không phải mock. Slice này harden ownership để chỉ kỹ thuật viên đang phụ trách mới được `release/status/battery/logs`, chặn claim đè lên người khác, tự ghi audit log khi cập nhật pin, và trim + bắt buộc note không rỗng cho drone logs. Đồng thời thêm `POST /api/maintenance/drone-orders/{orderId}/cancel` nhận body `{reasonCode, note?}`; chỉ cho hủy khi order đang `deliveryStage=ACCEPTED` và mission `READY_TO_LAUNCH`, yêu cầu note khi `reasonCode=5 (OTHER)`, release box giữ chỗ, xóa mission, và chốt order sang `CANCELED`.

**Payment gate trước pickup (2026-07-14):** với `DRONE_DELIVERY`, payment event chỉ còn đánh dấu `paymentStatus=PAID`, `paidAt`; **không** còn ép order sang queue dispatch. Queue/accept/launch chạy được khi order vẫn `UNPAID`. Đổi lại, khi phase sau đặt `deliveryStage=READY_FOR_PICKUP`, các đường truy cập pickup (`getByPin` / `getByAccess` và `complete`) sẽ chặn nếu `paymentStatus!=PAID`, trả lỗi `DRONE_PAYMENT_REQUIRED_BEFORE_PICKUP`. Mobile customer cũng phải ẩn/chặn `Mở tủ`, `Hoàn tất`, `Ủy quyền` cho drone chưa thanh toán và chỉ giữ nút `Thanh toán` cho tới khi pickup gate được mở.

**Demo tracking backend-owned (2026-07-14):** migration **`V9__drone_demo_tracking.sql`** thêm `orders.fulfillment_mode` và snapshot `drone_missions.drone_code`. Quyền tạo DEMO được điều khiển bằng `APP_DRONE_DEMO_ENABLED` và allowlist `APP_DRONE_DEMO_ALLOWED_USER_IDS`; request không truyền mode chỉ mặc định DEMO khi user được phép, còn lại về STANDARD, và yêu cầu DEMO trái quyền bị từ chối trước khi reserve ô. Sau khi MAINTENANCE launch một order `DEMO`, scheduler trong order-service (delay mặc định 7 giây/stage, cấu hình qua env) tự tiến `LAUNCHING -> DEPARTED -> EN_ROUTE -> APPROACHING -> ARRIVED -> READY_FOR_PICKUP`. Ở mốc cuối, mission thành `DEPOSITED`, order thành `STORING`, sinh PIN + hạn pickup 24 giờ và yêu cầu fleet đưa drone về `IDLE`; lỗi đồng bộ fleet/notification không rollback kiện hàng đã giao. `GET /api/orders/{orderId}/drone-delivery` là read model owner-safe ghép order + mission, trả `fulfillmentMode`, mission/drone/source/destination, `deliveryStage` và ETA. Mobile bật endpoint thật, poll mỗi 3 giây và render timeline 8 mốc; không còn mock cho timeline này.

**Notification contract hiện tại (2026-07-14):** lúc tạo đơn, order-service fan-out best-effort type **`DRONE_ORDER_CREATED`** tới user ACTIVE có role MAINTENANCE; tap mở `/maintenance-home`. Customer owner nhận type **`DRONE_DELIVERY_STATUS_CHANGED`** ở các mốc quan trọng `ACCEPTED`, `DEPARTED`, `APPROACHING`, `ARRIVED`, `READY_FOR_PICKUP`; tap mở `DroneDeliveryTrackingPage(orderId)`. Không push mỗi lần scheduler chạy; polling là nguồn cập nhật UI chính. Mobile vẫn nhận các type `drone_*` cũ trong giai đoạn chuyển tiếp.
...
Còn thiếu: simulator hiện chỉ tiến **stage**, chưa có vị trí/GPS/live map; iot-service chưa gọi position publisher; authorization per-order trên topic `/topic/**` chưa siết; chưa có MAVLink mission upload, telemetry thật, arrival/drop sensor hay locker-open tự động. Các phần này thuộc flow production tiếp theo, không được suy diễn từ demo scheduler.
