...
**Phase 1 — Maintenance queue + mission foundation (2026-07-14):** queue điều phối mới dùng **`orderId`** thay vì `DroneDeliveryRequest.id`: `GET /api/maintenance/drone-orders`, `POST /api/maintenance/drone-orders/{orderId}/accept`, `POST /api/maintenance/drone-orders/{orderId}/launch`. Từ slice `feat/drone-pay-before-pickup`, `accept` **không còn yêu cầu `paymentStatus=PAID`**; payment được dời sang checkpoint trước pickup. `accept` tạo/ghi `drone_missions` (migration **`V8__drone_mission_foundation.sql`**) và chạy preflight skeleton qua internal calls sang `locker-service`: drone phải `active=true`, `status=IDLE`, pin `>20%`; locker đích phải `ACTIVE`, có `landingPad=true`, `landingPadStatus=OK`. `launch` hiện mới là skeleton: đổi mission sang `LAUNCHING`, order sang `deliveryStage=LAUNCHING`, và yêu cầu locker-service đổi drone sang `IN_FLIGHT` qua `/internal/drones/{id}/status`. Gateway thêm route riêng `/api/maintenance/drone-orders/** -> order-service` trước route maintenance chung. Mobile MAINTENANCE đã được cắt sang queue mới với 3 nhóm `AWAITING_DISPATCH` / `ACCEPTED` / `LAUNCHING` và action `Tiếp nhận` / `Phóng`. Legacy `/api/maintenance/drone-deliveries` vẫn còn tạm cho dữ liệu/flow cũ nhưng không còn là queue chính cho maintenance Phase 2.
...
Còn thiếu: iot-service chưa gọi publisher (hiện test bằng gọi thẳng internal endpoint); `GET /api/orders/{orderId}/drone-delivery` mới chỉ là foundation read model, **chưa** có timeline/tracking/live position; authorization per-order trên topic `/topic/**` chưa siết (mọi user đã đăng nhập có thể subscribe orderId bất kỳ — chấp nhận ở Phase 2, siết sau nếu cần). Gắn với "Drone delivery service đầy đủ" (mục 11.1: battery-aware assignment, điều khiển bay thật).
